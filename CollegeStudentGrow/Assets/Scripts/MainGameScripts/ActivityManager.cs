...
public class ActivityManager : MonoBehaviour
{
    ...
    private void UpdateUI()
    {
        ...
        if (player.score >= 99)
            fillImage.color = Color.green; // A+일 경우
        else if (player.score >= 87.5)
            fillImage.color = Color.green; // A일 경우
        else if (player.score >= 75)
            fillImage.color = Color.green; // B+일 경우
        else if (player.score >= 62.5)
            fillImage.color = Color.green; // B일 경우
        else if (player.score >= 50)
            fillImage.color = Color.green; // C+일 경우
        else if (player.score >= 37.5)
            fillImage.color = Color.green; // C일 경우
        else if (player.score >= 25)
            fillImage.color = Color.green; // D+일 경우
        else if (player.score >= 12.5)
            fillImage.color = Color.green; // D일 경우
        else
            fillImage.color = Color.green; // F일 경우
    }
